...
**Anthropic (OAuth)**: When authenticated via OAuth (Claude Pro/Max subscription), this library calls the Anthropic Messages API **directly** using `@anthropic-ai/sdk` — it does **not** use the `@anthropic-ai/claude-code` subprocess SDK. The OAuth access token (`sk-ant-oat-*`) is passed as a Bearer token (`authToken`) to the SDK. To satisfy the requirements of the OAuth grant, Claude Code-compatible request headers are included automatically (`anthropic-beta: claude-code-20250219,oauth-2025-04-20`, `user-agent: claude-cli/<version>`, `x-app: cli`) and a Claude Code identity system prompt is prepended. This ensures Anthropic's backend recognizes the request as originating from an authorized CLI client and applies the Claude Pro/Max subscription entitlements correctly.
